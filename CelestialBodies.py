...
class CelestialBodies:
    ...
    def __init__(self, mass=.0000000001, radius=0, velocity=(0,0,0), position=(0,0,0), name="System", color="red"):
        ...
        # Position will be input in units of km, we will change to meters when calculating distance
        self.position = position #km
        self.delta_time = 86400 # seconds in a day so the simulation updates on one earth day intervals
    ...
    def distance_to(self, other):
        ...
        dist_list = [(s - o)**2 for s,o in zip(self.position, other.position)] #squared distances
        dist = np.sqrt(np.sum(dist_list)/10000)*100 #distance magnitude # the 10000 and 100 are used to allow python to actually do the calculation because some numbers are too big to take the sqrt of
